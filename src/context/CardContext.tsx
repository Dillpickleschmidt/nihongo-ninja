...
import debounce from "@/lib/flashcards/debounce"
...
export function CardProvider({
  ...
  const handleChange = function (
    ...
    if (nextDue) {
      note.card.due = nextDue.toISOString() // cast Date to ISOString
    }
  ...
  const handleSchdule = debounce(async (grade: Grade) => {
    ...
    const res = await fetch(
      `/api/flashcards?cid=${note.card.cid}&now=${new Date()}&grade=${grade}`,
      {
        method: "put",
      }
    ).then((res) => res.json())
  ...
  useEffect(() => {
    const note = noteBox[currentType][0]
    if (note) {
      fetch(`/api/fsrs?cid=${note.card.cid}&now=` + new Date(), {
        method: "post",
      })
        .then((res) => res.json())
