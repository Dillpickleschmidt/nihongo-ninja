...
import type { VocabularyItem, RichVocabItem } from "@nn/convex/validators";
import { extractHiragana, convertFuriganaToRubyHtml } from "@nn/data/utils/text/furigana";
...
export function addKanaAndRuby(
  ...
  return items.map((item) => {
    ...
    const finalHiragana = removeDuplicateKana && hiragana === item.word ? "" : hiragana;
