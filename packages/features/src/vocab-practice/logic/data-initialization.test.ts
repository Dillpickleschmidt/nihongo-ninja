...
const createMockRadical = (radical: string, meanings: string[]): RadicalEntry => ({
  radical,
  meanings,
  meaningMnemonic: `Mnemonic for ${radical}`,
});
...
const createMockKanji = (
  ...
  meanings,
  radicalComponents,
  meaningMnemonic: `Mnemonic for ${kanji} meaning`,
  readingMnemonic: `Mnemonic for ${kanji} reading`,
});
...
const createMockFSRSCard = (
  ...
  practiceItemKey: key,
  card: { ...createEmptyCard(new Date()), state },
  mode,
  type,
});
...
describe("Data Initialization", () => {
  describe("hierarchy processing and card creation", () => {
    it("should create cards for all hierarchy items with correct keys and types", () => {
      ...
      expect(result.cardMap.get("vocabulary:食べる")?.practiceItemType).toBe("vocabulary");
  ...
  describe("dependency management", () => {
    it("should build correct dependency relationships", () => {
      ...
      expect(result.dependencyMap.get("vocabulary:食べる")).toContain("kanji:食");
    ...
    it("should lock cards with dependencies and unlock cards without", () => {
      ...
      const unlockedCards = ["radical:人", "radical:口", "radical:水", "kanji:写", "kanji:真"];
      ...
      const edgeCases = [
        ...
        },
        {
          ...
          expectation: (result: ReturnType<typeof initializePracticeSession>) => {
            ...
            expect(result.cardMap.get("vocabulary:due1")?.sessionStyle).toBe("flashcard");
          },
        },
      ];

      for (const { hierarchy, moduleData, nonModuleData, expectation } of edgeCases) {
        const result = initializePracticeSession(
  ...
  describe("FSRS data handling", () => {
    it("should preserve and filter FSRS data correctly", () => {
      ...
      expect(vocabCard?.fsrs.logs).toHaveLength(0);
    ...
    it("should filter incoming FSRS data by the current session mode", () => {
      ...
      mixedModeFSRSCards[1]!.card.stability = 50;
      mixedModeFSRSCards[2]!.card.stability = 20;
      mixedModeFSRSCards[3]!.card.stability = 60;
    ...
    it("should assign correct session styles based on card state and type", () => {
      const testCases = [
        ...
        },
        {
          ...
          moduleData: createModuleData(mockVocabularyItems, mockKanjiEntries, mockRadicalEntries, [
            createMockFSRSCard("食べる", "meanings", "vocabulary", State.Review),
          ]),
          nonModuleData: createNonModuleData(),
          expectations: [{ key: "vocabulary:食べる", style: "multiple-choice" }],
        },
        {
  ...
  describe("queue initialization", () => {
    it("should initialize queues correctly", () => {
      ...
      expect(result.moduleQueue).toEqual(
        expect.arrayContaining(["radical:人", "radical:口", "radical:水", "kanji:写", "kanji:真"]),
      );
      expect(result.reviewQueue).toHaveLength(0);
      ...
      const fsrsCardsWithFutureDates: FSRSCardInput[] = [
        {
          ...createMockFSRSCard("人", "meanings", "radical", State.Review),
          card: {
            ...createMockFSRSCard("人", "meanings", "radical", State.Review).card,
            due: futureDate,
          },
        },
        {
          ...
          card: {
            ...createMockFSRSCard("口", "meanings", "radical", State.Review).card,
            due: futureDate,
          },
        },
      ];
  ...
  describe("complex dependency chains", () => {
    it("should handle complex dependency chains", () => {
      ...
      const complexVocabRelationship: VocabRelationship = createMockVocabRelationship("複雑", [
        "複",
        "食",
      ]);
      const complexKanjiRelationship: KanjiRelationship = createMockKanjiRelationship("複", [
        "人",
        "口",
        "水",
      ]);
      ...
      const result = initializePracticeSession(
        complexHierarchy,
        createModuleData(complexVocabularyItems, complexKanjiEntries, mockRadicalEntries),
        createNonModuleData(),
        "meanings",
        false,
        true,
  ...
  describe("prompt and validAnswers correctness", () => {
    ...
    it("should never have prompt equal to any validAnswer", () => {
      ...
      for (const [, card] of result.cardMap.entries()) {
  ...
  describe("review queue filtering", () => {
    ...
    it("should exclude review cards when includeReviews is false", () => {
      ...
      const result = initializePracticeSession(
        mockHierarchy,
        createModuleData(),
        createNonModuleData([...mockVocabularyItems, externalVocab], [], [], [dueReviewCard]),
        "meanings",
        false,
        true,
